...
export default function ValidateNamePage() {
  ...
  // If no direct sales found, user can proceed to home
  if (!directSaleInfo?.hasOrders) {
    return null;
  }

  // If only one name, auto-redirect to orders page
  if (directSaleInfo.customerNames.length === 1) {
    router.push('/orders');
    return null;
  }
